...
# /yb-pr-flow-review が完了していないブランチのpushを物理的に防止する
set -euo pipefail
...
# フラグファイルの存在を確認
if [[ ! -f "$flag_file" ]]; then
  echo "ブロック: レビューが未完了です。/yb-pr-flow-review を実施してからpushしてください。" >&2
  exit 2
fi
...
if (( elapsed > 3600 )); then
  echo "ブロック: レビュー結果が期限切れです（1時間超過）。/yb-pr-flow-review を再実施してください。" >&2
  exit 2
fi
