...
from auth import create_client, load_env
...
def generate_icon(prompt, style=None, category=None, name=None,
    ...
    if not _GENAI_AVAILABLE:
        ...
        print("  または --no-api フラグでAPI不要モードを使用してください。")
        sys.exit(1)
...
def main():
    ...
    parser.add_argument("--no-api", action="store_true",
                        help="API未使用モード: プロンプトとスタイル情報のみをJSON出力（LLMに渡してSVG生成可能）")
    ...
    # --no-api モード: プロンプトとスタイル情報のみをJSON出力
    if args.no_api:
        # プロンプトを組み立て（generate_iconと同じロジック）
        style_instructions = ""
        if args.style and args.style in ICON_STYLES:
            style_instructions = f"- Style: {ICON_STYLES[args.style]}"

        color_instructions = "- Use currentColor for all strokes and fills"
        if args.color:
            color_instructions = f"- Use color: {args.color} for primary elements, currentColor for secondary"

        size = args.size if hasattr(args, 'size') else 24
        size_instructions = f"- Design for {size}px display size, optimize detail level accordingly"

        icon_prompt = prompt
        if args.category and args.category in ICON_CATEGORIES:
            icon_prompt = f"{prompt} (category: {ICON_CATEGORIES[args.category]})"
        if args.name:
            icon_prompt = f"'{args.name}' icon: {icon_prompt}"

        viewbox = args.viewbox if hasattr(args, 'viewbox') else 24
        ...
        output = {
            "mode": "no-api",
            "prompt": full_prompt,
            "style": args.style or "outlined",
            "color": getattr(args, 'color', None),
            "sizes": args.sizes if args.sizes else None,
            "svg_guidelines": "viewBox属性必須、ラスター画像不可、パス数は最小限に。stroke-width統一、fill/stroke属性で色指定。",
            "message": "このJSONをLLM（Claude等）に渡してSVGを生成してください"
        }
