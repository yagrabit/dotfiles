    ...
    try {
      for (const rule of sheet.cssRules) {
        if (rule instanceof CSSMediaRule) {
          ...
          const widthMatch = text.match(/(min|max)-width:\s*([\d.]+)(px|em|rem)/);
          if (widthMatch) {
            const [, type, value, unit] = widthMatch;
            const px = unit === 'px' ? parseFloat(value)
                     : unit === 'em' || unit === 'rem' ? parseFloat(value) * 16
                     : parseFloat(value);
            const key = `${type}-width: ${px}px`;
            breakpoints.set(key, (breakpoints.get(key) || 0) + rule.cssRules.length);
          }
        }
      }
    } catch(e) {}
  }
  return Object.fromEntries([...breakpoints.entries()].sort((a, b) => b[1] - a[1]).slice(0, 15));
})()
```
...
1. 抽出したメディアクエリのブレークポイント値をpxに正規化してソート
2. 一般的なブレークポイント範囲にマッピング:
   - ~479px: Small Mobile
   - 480-639px: Mobile
...
3. 各ブレークポイントで変化するレイアウト・タイポグラフィの特徴を記録
...
| サイト名 | URL | 業界 | モード | 分析日 | レポートパス | ショーケースパス |
|---------|-----|------|-------|-------|-------------|---------------|
```

既存ファイルがある場合は行を追加する（レポートパスとショーケースパスの両方を記録）。

---

## Phase 5: Showcase（HTMLショーケース生成）

[showcase-spec.md](showcase-spec.md) を読み込み、Phase 2の構造化データとPhase 4のMDレポートからHTMLショーケースを生成する。

### 5-1. データ変換

Phase 2（Structure）の構造化データとPhase 4（Report）のMDレポートから、CSS変数セットを構築する。

#### auditモード

- MDレポートの「改善後デザイントークン案」テーブルから改善後の値を抽出し、`:root` のCSS変数として使用
- 改善案がないトークン（維持）は現状値をそのまま使用
- 現状値は Before-After タブの Before ペインで使用（インラインstyleで直接指定）

#### learnモード

- Phase 2の構造化データをそのまま `:root` のCSS変数として使用
- Before-After タブは生成しない

#### ダークモードトークン

- テーマ検出（Phase 1-3.5）で2回抽出がある場合: 対テーマの抽出値を `[data-theme="dark"]` に使用
- テーマ未検出の場合: showcase-spec.md のダークモード自動生成ルールに従い、ライトモード値から変換

### 5-2. HTML生成

showcase-spec.md の仕様に従い、3タブ構成のHTMLを生成する:

- Tab 1 (Tokens): カラースウォッチ / タイプスケール / スペーシングチャート / エフェクト / 問題一覧（auditのみ）
- Tab 2 (Before-After): auditモードのみ。ドラッグスライダーで改善前後を比較
- Tab 3 (Preview): トークンを適用したSaaS LPテンプレート。テキストコンテンツは対象サイトの業界・サービスに合わせて調整

learnモードの場合:
- Tab 2のタブボタンとコンテンツを生成しない
- テーマ検出なしの場合、テーマトグルボタンも生成しない

### 5-3. 出力

出力先: `.claude/artifacts/showcase-{site-name}-{yyyyMMdd-HHmm}.html`

タイムスタンプはPhase 4のMDレポートと同じものを使用する。

### 5-4. モード別動作

| 要素 | learn | audit |
|------|-------|-------|
| Tab 1: Tokens | 抽出トークン表示 | 改善後トークン + 問題一覧 |
| Tab 2: Before-After | 非表示 | スライダー比較 |
...
- 成果物ファイルのパス（MDレポート + HTMLショーケースの両方）
