...
PostToolUseフック（`post-skill-auto-review-reminder.sh`）がSkillツール完了後に自動発火し、実行すべきauto系スキルをリマインドする。フックの指示は必須であり、スキップしてはならない。
...
2. PASS判定後に次のステップへ進む。REJECT時はFindingベースのフェーズルーティング（§ 5-3.7）で自動差し戻し
...
4. odin-talk-review — 必須。code-reviewer + security-reviewer + Codex による複数モデルクロスレビューで品質を担保する。次のWaveでodin-do-prを実行する場合もスキップしない
5. odin-auto-verify（最終検証） — 必須

### レビューチェーンの意義

推測せず調査→計画→レビュー→実行のサイクルを回すことで、ユーザー承認なしでも高い品質を維持する。特にCodexによる異なるモデルでのレビューは、単一モデルでは検出できない問題を発見する重要な品質ゲートである。
...
有用な知見がある場合:
- Skillツールで `odin-knowledge` (ingestモード) を実行する
- 引数: 成果物ファイルパスを指定
- odin-knowledgeがユーザーに取り込む知見を確認してからknowledgeに統合する
