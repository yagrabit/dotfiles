---
name: odin
description: 開発ワークフローのオーケストレーター。ユーザーの要望を分析し、配下23スキルから最適な組み合わせを選んでオーケストレーションする。あらゆる開発シーンに対応し、自己進化する。「/odin」で起動。
user-invocable: true
...
ユーザーの自由形式の入力（テキスト、音声文字起こし、曖昧な要望）を受け取り、コンテキストを自動補完し、配下23スキルの最適な組み合わせでタスクを実行する。
...
3. コンテキスト管理: 2 Wave以上経過した場合、以下のいずれかを選択する
   ...
   b. ハードリセット（context reset）— 長時間タスク用:
      - Anthropic公式原則: コンテキストを完全にクリアし、新しいエージェントを起動する
      - 適用条件: 5 Wave以上経過、またはコンテキストが著しく肥大化している場合
      - 手順:
        1. 現時点の進捗を `.claude/artifacts/checkpoint-{yyyyMMdd-HHmm}.md` に保存する
           内容: 完了タスク一覧、残タスク一覧、各成果物ファイルパス、次のWaveの説明
        2. ユーザーに「コンテキストが大きくなったため、新しいセッションで継続することを推奨します」と報告する
        3. ユーザーが新セッションを開始した際、checkpointファイルを読み込んで残タスクから再開する
      - 注意: Opus 4.6ではcontext anxiety問題は大幅に軽減されており、ハードリセットの必要性は低い。明らかにコンテキストが限界に近い場合のみ使用する
4. 次のWaveの実行を開始する
