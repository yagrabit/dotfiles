...
- PRサイズガイドラインの数値基準を参照する

検証コントラクトの作成:
- 各タスクに対して、完了を判定するための具体的・測定可能な基準を定義する
- 基準は客観的に判定可能な表現にする（「良い」「きれい」等の主観表現は禁止）
- 検証方法を明記する（テスト実行、ビルド成功、lint通過、エンドポイントの応答等）
- UI系タスクにはビジュアル基準も含める（レスポンシブ対応、コンソールエラーなし等）
- コントラクトはodin-auto-verifyが自動チェックする前提で記述する
...
- 全タスクに検証コントラクト（具体的・測定可能な完了基準）が付与されていること
...
## 検証コントラクト（Verification Contract）

Anthropic公式原則: "Before each sprint, the generator and evaluator negotiated a sprint contract: agreeing on what 'done' looked like."

各タスクに対して、odin-auto-verifyがチェックする具体的・測定可能な完了基準を定義する。

### コントラクトの要件
- 各基準は客観的に判定可能であること（「良い」「きれい」等の主観表現は禁止）
- 具体的な検証方法を明記すること（テスト実行、ビルド成功、lint通過等）
- UI系タスクの場合、ビジュアル基準も含めること（レスポンシブ対応、コンソールエラーなし等）

### 出力フォーマット例

Wave形式の計画に検証コントラクトを付加する:

```
Wave 2（Wave 1完了後、並列可能）:
  T-04: [odin-do-implement] 通知API実装(TDD)
    検証コントラクト:
    - [ ] POST /api/notifications エンドポイントが201を返す
    - [ ] GET /api/notifications がユーザー別の通知一覧を返す
    - [ ] 未読通知の件数取得APIが正しくカウントする
    - [ ] テスト: 通知CRUD操作の全パターンがパス
    - [ ] テスト: 認証なしアクセスで401が返る
    - [ ] ビルド: tsc --noEmit が成功する

  T-05: [odin-do-implement] 通知UIコンポーネント実装(TDD)
    検証コントラクト:
    - [ ] ベルアイコンクリックでドロップダウンが開閉する
    - [ ] 未読件数がバッジで表示される
    - [ ] 通知クリックで既読状態になる
    - [ ] テスト: コンポーネントの主要操作がパス
    - [ ] lint: oxlint が警告0で通過
    - [ ] レスポンシブ: 390px幅で表示が崩れない
```

### odin-auto-verifyとの連携

- 検証コントラクトは `.claude/artifacts/plan-*.md` に含まれる
- odin-auto-verifyはタスク完了時にコントラクトの各項目をチェックする
- 全項目PASSで完了、1項目でもFAILなら未完了として報告
...
ステップ3: 依存関係・Wave + 検証コントラクト
  Wave 1: T-001
  Wave 2: T-002, T-003（並列）
    T-002 検証コントラクト:
      - [ ] 通知作成APIのテストが定義されている
      - [ ] テスト実行で全件FAILする（Red状態）
    T-003 検証コントラクト:
      - [ ] 通知一覧取得APIのテストが定義されている
      - [ ] テスト実行で全件FAILする（Red状態）
  Wave 3: T-004, T-005（並列）
    T-004 検証コントラクト:
      - [ ] POST /api/notifications が201を返す
      - [ ] テスト: T-002で作成した全テストがパス
      - [ ] ビルド: tsc --noEmit が成功する
    T-005 検証コントラクト:
      - [ ] GET /api/notifications がユーザー別の通知一覧を返す
      - [ ] テスト: T-003で作成した全テストがパス
      - [ ] ビルド: tsc --noEmit が成功する
  Wave 4: T-006
  Wave 5: T-007
    T-007 検証コントラクト:
