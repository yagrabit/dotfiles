...
- odinから自動呼び出しされている
- ステップ1（変更の確認）から開始する
- コミットメッセージのユーザー承認（ステップ4）はスキップし、odinの判断で自動コミットする
- ただしセキュリティチェック（ステップ2）は必ず実行する
...
1. `git status --short` を実行し、変更の有無を確認する:
   ```bash
   git status --short
   ```
2. 出力が空の場合（変更がない場合）は「コミットする変更がありません」とユーザーに報告してスキルを終了する
3. 変更がある場合のみステップ1以降に進む
...
2. pre-commitフックが失敗した場合:
   a. フックのエラー出力を確認し、問題を特定する
   b. 自動修正可能な場合（lint、フォーマット等）は修正を適用する
   c. 修正後、変更を再ステージングし、新しいコミットを作成する（--amendは使用しない）
   d. 自動修正不可能な場合はユーザーに報告する
   e. 最大3回までリトライし、解決しない場合はユーザーにエスカレーションする
