...
- マージ方法の選択とマージ実行はユーザーの明示的承認が必須。Squash mergeをデフォルト推奨として提示する
- CI/レビュー承認の確認（ステップ1）は必ず実行する
...
- CI全チェック通過が必須。CI失敗時はマージを実行しない（例外なし）
- レビュー承認（APPROVED）が必須。未レビュー・CHANGES_REQUESTEDの場合はマージを実行しない（例外なし）
- `git push --force` / `git push --force-with-lease` を禁止する
- `git branch -D`（強制削除）はAskUserQuestionでユーザーの明示的承認を得てから実行する（既存ルール）
...
3. CI（継続的インテグレーション）の状態を確認する:
   ...
   - 実行中（pending）→ 完了を待つ。AskUserQuestionで「待機する/中断する」を確認する
   - 失敗（failure）→ マージを拒否する。原因をユーザーに報告し、修正を案内する（CI未通過のままマージは実行しない）
...
4. レビュー承認状態を確認する:
   ...
   レビュー状態の判定:
   - 承認済み（APPROVED）→ マージ可能
   - 変更要求あり（CHANGES_REQUESTED）→ マージを拒否する。変更要求の内容をユーザーに報告し、修正を案内する
   - 未レビュー → マージを拒否する。「レビュー承認が必要です」とユーザーに報告する
...
odinコンテキスト経由でも、マージ実行はユーザーの明示的承認が必須である。以下のAskUserQuestionは省略しない。
...
マージはステップ2でユーザーの明示的承認を得た場合にのみ実行する。承認なしにマージを実行してはならない。
