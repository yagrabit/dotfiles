...
| `cc-save` | Claude Codeの全セッション情報をバックアップファイルに保存 |
| `cc-restore` | バックアップからClaude Codeセッションをtmuxウィンドウとして一括復元 |
| `ob` | odin-boardタスク管理TUIのラッパー |
...
- cc-auto-restore: tmux起動時に前回のClaude Codeセッション復元を自動提案
- cd後にeza --icons自動表示
- zoxide, fzf, mise, starship の初期化（`_cached_init` によるキャッシュ付き）
...
| `Prefix \|` | 右にペイン分割（25%幅） |
| `Prefix -` | 下にペイン分割 |
| `Prefix \` | 右にClaude Code付きペイン分割（50%幅） |
| `Prefix _` | 下にClaude Code付きペイン分割 |
| `Prefix c` | 新しいウィンドウ |
| `Prefix C` | ワークスペース作成（ghqプロジェクト選択 + レイアウト選択のポップアップ） |
| `Prefix b` | odin-boardカンバンTUIをポップアップ表示 |
...
tmux本体・プラグインのデフォルトバインドを含めて未使用のキー: `u`, `v`, `y`（v/yはコピーモードでは使用済み。このテーブルはPrefixキーバインドでの未使用を示す）
...
| カテゴリ | 名前 | 説明 |
|---|---|---|
| odin | `/odin` | 開発ワークフロー司令塔。配下33スキルをオーケストレーションし、あらゆる開発シーンに対応する自己進化型スキル |
| odin/think系 | `/odin-think-research`, `-requirements`, `-design`, `-plan`, `-investigate`, `-analyze`, `-qa-design` | 調査・要件分析・設計・タスク分解・バグ調査・品質分析・QA試験設計の7種 |
| odin/do系 | `/odin-do-commit`, `-implement`, `-test`, `-refactor`, `-pr`, `-merge`, `-qa-execute` | コミット・実装・テスト・リファクタ・PR作成・マージ・QA試験実行の7種 |
| odin/talk系 | `/odin-talk-clarify`, `-propose`, `-review`, `-explain`, `-triage` | 要件ヒアリング・提案・コードレビュー・説明・タスク分類の5種 |
| odin/auto系 | `/odin-auto-quality`, `-review`, `-verify`, `-improve`, `-evolve`, `-coherence`, `-peer-review`, `-record` | 品質チェック・セルフレビュー・完了検証・自己改善・スキル自己進化・成果物整合性・ドキュメントレビュー・振り返り記録の8種 |
| odin/codex系 | `/odin-codex-search` | Codex経由Web検索（未対応環境はWebSearchフォールバック）の1種 |
| odin/learn系 | `/odin-learn`, `-learn-review` | 独立学習セッション・PRレビュー教育モードの2種 |
| odin/design系 | `/odin-design-dissect`, `-design-generate`, `-design-knowledge` | デザインシステム抽出・生成・ナレッジ管理の3種 |
| PR/Git | `/pr-worktree` | worktreeでの隔離作業 |
| PR/Git | `/pr-coderabbit-fix` | CodeRabbitレビュー指摘の分析・修正 |
| PR/Git | `/pr-attach-image` | ローカル画像をPlaywright MCP経由でGitHub PRにコメントとして添付 |
...
| UX/デザイン | `/odin-ux-design` | Gemini AIによるロゴ・CIPモックアップ・アイコン生成（55スタイル） |
| UX/デザイン | `/odin-ux-banner` | バナーデザイン（SNS・広告・Webヒーロー・印刷向け、22スタイル対応） |
| UX/デザイン | `/odin-ux-slides` | HTMLプレゼンテーション生成（Chart.js・コピーライティング公式・15デッキ戦略） |
...
| pre-push-review-gate | PreToolUse | `/odin-talk-review` 未完了のブランチからのpushをブロック |
...
| block-test-deletion | PreToolUse | テストファイル・テストコードの削除をブロック |
| suggest-compact | UserPromptSubmit | プロンプト回数をカウントし30回ごとに `/compact` 実行を提案 |
| quality-gate | PostToolUse | 編集ファイルに対してconsole.log検出・import重複・TODO過多の品質チェックを実行 |
| notification-tracker | Notification | 許可待ち/入力待ち状態を検知しセッション状態ファイルを更新 |
| post-skill-auto-review-reminder | PostToolUse | Skillツール完了後にauto系スキルの実行をリマインド |
...
| playwright | ブラウザ自動操作（スクリーンショット・操作・E2E） |
| codex | OpenAI Codex連携（条件付き） |
...
├── skills/            カスタムスキル（odin系33個・ux系7個・pr系・qa系・doc系等）
├── tools/             カスタムツール（yb-memory等）
├── private_plugins/   プラグイン設定
└── private_executable_statusline-command.py  ステータスライン表示スクリプト
dot_tmux.conf          → ~/.tmux.conf
docs/                  GitHub Pages用メトリクスダッシュボード
scripts/               メトリクス収集スクリプト
tests/                 odin-board等のテスト
```
